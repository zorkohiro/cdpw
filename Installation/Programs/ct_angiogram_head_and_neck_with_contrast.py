...
#
root=tk.Tk()
root.title("CT Angiogram Head and Neck with Contrast" + " MRN: " + mrn + " SESSION: " + session)
...
head_box, rowvar = add_label_box("CTA HEAD", json_data[FI]["head"], rowvar)
rowvar = add_label("CTA NECK", rowvar, 0)
vessels_box, rowvar = add_label_box("GREAT VESSELS", json_data[FI]["vessels"], rowvar)
right_ica_box, rowvar = add_label_box("RIGHT ICA", json_data[FI]["right_ica"], rowvar)
left_ica_box, rowvar = add_label_box("LEFT ICA", json_data[FI]["left_ica"], rowvar)
vertebral_box, rowvar = add_label_box("VERTEBRAL ARTERIES", json_data[FI]["vertebral"], rowvar)
...
def save_data(jd):
    ...
    jd[FI]["head"] = head_box.get("1.0", "end-1c")
    jd[FI]["vessels"] = vessels_box.get("1.0", "end-1c")
    jd[FI]["right_ica"] = right_ica_box.get("1.0", "end-1c")
    jd[FI]["left_ica"] = left_ica_box.get("1.0", "end-1c")
    jd[FI]["vertebral"] = vertebral_box.get("1.0", "end-1c")
...
def create_report(rpt):
    ...
    with open(rpt, "w") as ofile:
        ...
        print("CT Angiogram Head and Neck with Contrast", file=ofile)
        print("", file=ofile)
        ...
        print("CTA HEAD:".ljust(20) + json_data[FI]["head"], file=ofile)
        print("", file=ofile)
        print("CTA NECK".ljust(20), file=ofile)
        print("GREAT VESSELS:".ljust(20) + json_data[FI]["vessels"], file=ofile)
        print("RIGHT ICA:".ljust(20) + json_data[FI]["right_ica"], file=ofile)
        print("LEFT ICA:".ljust(20) + json_data[FI]["left_ica"], file=ofile)
        print("VERTEBRAL ARTERIES:".ljust(20) + json_data[FI]["vertebral"], file=ofile)
        print("OTHER:".ljust(20) + json_data[FI]["other"], file=ofile)
